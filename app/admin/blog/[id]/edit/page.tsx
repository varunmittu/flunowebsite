  ...
  return (
    <div className="flex-1 p-6 lg:p-10 bg-fluno-light min-h-screen">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="font-display text-2xl text-fluno-ink">Edit Post</h1>
          <p className="font-body text-sm text-fluno-muted mt-0.5">{form.title}</p>
        </div>
      </div>
      ...
      <form onSubmit={submit} className="space-y-6 max-w-3xl">
        ...
        <div className="bg-white rounded-2xl border border-fluno-lavender p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2"><label className="block font-body text-xs text-fluno-muted mb-1.5">Title *</label>
              <input value={form.title} onChange={e => set("title", e.target.value)} className="input text-lg font-display" required /></div>
            ...
            <div><label className="block font-body text-xs text-fluno-muted mb-1.5">Author</label>
              <input value={form.author} onChange={e => set("author", e.target.value)} className="input" /></div>
            <div><label className="block font-body text-xs text-fluno-muted mb-1.5">Read Time</label>
              <input value={form.readTime} onChange={e => set("readTime", e.target.value)} className="input" /></div>
            <div className="col-span-2"><label className="block font-body text-xs text-fluno-muted mb-1.5">Cover Image URL</label>
              <input type="url" value={form.coverImage} onChange={e => set("coverImage", e.target.value)} className="input" /></div>
            <div className="col-span-2"><label className="block font-body text-xs text-fluno-muted mb-1.5">Excerpt</label>
        ...
        <div className="bg-white rounded-2xl border border-fluno-lavender p-5">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <div><p className="font-body text-sm font-medium text-fluno-ink">Published</p>
              <p className="font-mono text-[10px] text-fluno-muted">Post is visible on the blog</p></div>
          </label>
        </div>
