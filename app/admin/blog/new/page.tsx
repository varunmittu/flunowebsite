  ...
  return (
    <div className="flex-1 p-6 lg:p-10 bg-fig-cream min-h-screen">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="font-fig font-bold text-2xl text-fig-navy">New Blog Post</h1>
      </div>
      ...
      <form onSubmit={submit} className="space-y-6 max-w-3xl">
        ...
        <div className="bg-white rounded-2xl border border-fig-sage p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="block font-fig-body text-xs text-fig-ink-soft mb-1.5">Title *</label>
              <input value={form.title} onChange={e => { set("title", e.target.value); set("slug", toSlug(e.target.value)); }} className="input text-lg font-fig font-bold" placeholder="Post title..." required />
            </div>
            <div>
              <label className="block font-fig-body text-xs text-fig-ink-soft mb-1.5">Slug *</label>
              <input value={form.slug} onChange={e => set("slug", toSlug(e.target.value))} className="input font-mono text-sm" required />
            </div>
            <div>
              <label className="block font-fig-body text-xs text-fig-ink-soft mb-1.5">Category</label>
              <input value={form.category} onChange={e => set("category", e.target.value)} className="input" placeholder="Skincare / Sunscreen / etc." />
            </div>
            <div>
              <label className="block font-fig-body text-xs text-fig-ink-soft mb-1.5">Author</label>
              <input value={form.author} onChange={e => set("author", e.target.value)} className="input" />
            </div>
            <div>
              <label className="block font-fig-body text-xs text-fig-ink-soft mb-1.5">Read Time</label>
              <input value={form.readTime} onChange={e => set("readTime", e.target.value)} className="input" placeholder="5 min read" />
            </div>
            <div className="col-span-2">
              <label className="block font-fig-body text-xs text-fig-ink-soft mb-1.5">Cover Image URL</label>
              <input type="url" value={form.coverImage} onChange={e => set("coverImage", e.target.value)} className="input" placeholder="https://..." />
            </div>
            <div className="col-span-2">
              <label className="block font-fig-body text-xs text-fig-ink-soft mb-1.5">Excerpt</label>
        ...
        <div className="bg-white rounded-2xl border border-fig-sage p-5">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <div>
              <p className="font-fig-body text-sm font-medium text-fig-navy">Publish immediately</p>
              <p className="font-mono text-[10px] text-fig-ink-soft">Post will be visible on the blog</p>
            </div>
          </label>
        </div>
